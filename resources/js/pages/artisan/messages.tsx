...
import { MessageSquare, ArrowLeft, Search, Circle, Send } from 'lucide-react';
...
const breadcrumbs: BreadcrumbItem[] = [
    ...
    { title: 'Messages', href: '/artisan/messages' },
];
...
export default function ArtisanMessages({ conversations = [], selectedConversation = null, messages = [] }: Props) {
    ...
    useEffect(() => {
        ...
        const fetchMessages = async () => {
            try {
                const res = await fetch(route('artisan.messages.updates', { withUser: selectedConversation.id }));
                if (!res.ok) return;
    ...
    const handleSend: FormEventHandler = async (e) => {
        ...
        const res = await fetch(route('artisan.messages.send', { withUser: selectedConversation.id }), {
            method: 'POST',
            body: formData,
            headers: {
        ...
        window.location.href = route('artisan.messages', { withUser: selectedConversation.id });
    };
            ...
            <div className="flex flex-col gap-6 p-6 bg-[hsl(36,33%,97%)] min-h-screen">
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p className="mt-1 text-[hsl(20,10%,50%)]">Vos conversations avec les clients</p>
                    </div>
                </div>
                        ...
                        <div className="p-4 border-b border-[hsl(30,20%,88%)]">
                            <div className="relative">
                                ...
                                <input
                                    ...
                                    className="w-full rounded-xl border border-[hsl(30,20%,82%)] bg-white pl-10 pr-4 py-2 text-sm text-[hsl(20,14%,12%)] focus:border-amber-400 focus:outline-none"
                                />
                            </div>
                        </div>
                        <div className="overflow-y-auto flex-1">
                            {conversations.length === 0 ? (
                                ...
                                conversations.map((conv) => (
                                    <Link
                                        key={conv.id}
                                        href={route('artisan.messages', { withUser: conv.id })}
                                    ...
                                    >
                                        ...
                                        <div className="flex-1 min-w-0">
                                            <div className="flex items-center justify-between">
                                                <p className="font-semibold text-[hsl(20,14%,12%)] truncate">{conv.client_nom}</p>
                                                <span className="text-xs text-[hsl(20,10%,50%)] shrink-0">{conv.date}</span>
                                            </div>
                                            <p className="text-sm text-[hsl(20,10%,50%)] truncate mt-0.5">{conv.dernier_message}</p>
                                        </div>
                                        {conv.non_lus > 0 && (
                        ...
                        <div className="rounded-2xl border border-[hsl(30,20%,88%)] bg-white shadow-sm h-full flex flex-col">
                            {selectedConversation ? (
                                <>
                                    ...
                                    <div className="border-b border-[hsl(30,20%,88%)] px-6 py-4">
                                        <div className="flex items-center justify-between gap-4">
                                            ...
                                            <div className="text-xs text-[hsl(20,10%,50%)]">{messages.length} {messageCountLabel(messages.length)}</div>
                                        </div>
                                    </div>
                                            ...
                                            messagesState.map((message) => (
                                                <div key={message.id} className={`flex items-end gap-3 ${message.isMine ? 'justify-end' : 'justify-start'}`}>
                                                    ...
                                                    <div className={`relative max-w-[70%] space-y-1 px-5 py-3 shadow-sm ${message.isMine ? 'bg-[hsl(20,14%,12%)] text-white rounded-tl-3xl rounded-tr-3xl rounded-bl-3xl' : 'bg-[hsl(36,30%,93%)] text-[hsl(20,14%,12%)] rounded-tl-3xl rounded-tr-3xl rounded-br-3xl'}`}>
                                                        ...
                                                        <div className={`mt-2 text-[11px] ${message.isMine ? 'text-amber-300/80' : 'text-[hsl(20,10%,50%)]'}`}>
                                                            {message.date}
                                                        </div>
                                                        {message.call_session_id && (message.type === 'call_audio' || message.type === 'call_video') && (
                                                            <Link
                                                                href={route('artisan.calls.show', { callSession: message.call_session_id })}
                                    ...
                                    <form onSubmit={handleSend} className="border-t border-[hsl(30,20%,88%)] px-6 py-4">
                                        <div className="mb-3 flex flex-wrap items-center gap-3">
                                            <label className="inline-flex cursor-pointer items-center rounded-xl border border-[hsl(30,20%,82%)] bg-white px-4 py-2 text-sm text-[hsl(20,14%,12%)] shadow-sm transition hover:bg-[hsl(36,33%,97%)]">
                                                <input
                                                    type="file"
                                                    accept="image/*,audio/*"
                                                    className="hidden"
                                            ...
                                            <button
                                                ...
                                                className="rounded-xl border border-[hsl(30,20%,82%)] bg-white px-4 py-2 text-sm text-[hsl(20,14%,12%)] hover:bg-[hsl(36,33%,97%)] transition-colors"
                                            >
                                                Envoyer ma position
                                            </button>
                                            <button
                                                type="button"
                                                onClick={() => handleSendCall('call_audio')}
                                                className="rounded-xl border border-[hsl(30,20%,82%)] bg-white px-4 py-2 text-sm text-[hsl(20,14%,12%)] hover:bg-[hsl(36,33%,97%)] transition-colors"
                                            >
                                                Appel audio
                                            </button>
                                            <button
                                                type="button"
                                                onClick={() => handleSendCall('call_video')}
                                                className="rounded-xl border border-[hsl(30,20%,82%)] bg-white px-4 py-2 text-sm text-[hsl(20,14%,12%)] hover:bg-[hsl(36,33%,97%)] transition-colors"
                                            >
                                                Appel vidéo
                                            </button>
                                        ...
                                        <div className="space-y-2">
                                            <textarea
                                                ...
                                                className="w-full min-h-[110px] rounded-xl border border-[hsl(30,20%,82%)] bg-white p-4 text-[hsl(20,14%,12%)] shadow-sm focus:border-amber-400 focus:outline-none"
                                            />
                                            {data.attachment && (
                                                <p className="text-sm text-[hsl(20,10%,50%)]">Fichier sélectionné : {data.attachment.name}</p>
                                            )}
                                            <p className="text-xs text-[hsl(20,10%,50%)]">Tapez votre message puis cliquez sur Envoyer.</p>
                                            {errors.contenu && <p className="text-sm text-red-600">{errors.contenu}</p>}
                                        </div>
                                        <div className="mt-4 flex justify-end">
                                            <button
                                                type="submit"
                                                disabled={sending || (data.contenu.trim() === '' && !data.attachment)}
                                                className="inline-flex items-center gap-2 rounded-xl bg-gradient-to-r from-amber-500 to-orange-500 hover:from-amber-400 hover:to-orange-400 text-white font-semibold px-5 py-2.5 shadow-sm transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                                            >
                                ...
                                <div className="flex flex-col items-center justify-center flex-1 p-12 text-center">
                                    ...
                                    <p className="text-[hsl(20,10%,50%)] max-w-sm">
                                        Choisissez une conversation pour répondre à vos clients.
                                    </p>
                                </div>
                            )}
                        </div>
