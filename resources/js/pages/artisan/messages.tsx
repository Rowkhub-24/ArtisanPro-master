            ...
            <div className="flex flex-col gap-6 p-6 bg-gradient-to-br from-slate-50 via-white to-blue-50 min-h-screen">
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p className="mt-1 text-gray-600">Vos conversations avec les clients</p>
                    </div>
                </div>
                    ...
                    <Card className="border-0 shadow-lg bg-white overflow-hidden flex flex-col">
                        <div className="p-4 border-b border-gray-100">
                            <div className="relative">
                                ...
                                <Input placeholder="Rechercher un client..." className="pl-10 border-gray-200" />
                            </div>
                        </div>
                                ...
                                conversations.map((conv) => (
                                    ...
                                    >
                                        <div className="relative shrink-0">
                                            <div className="flex h-12 w-12 items-center justify-center rounded-full bg-gradient-to-br from-indigo-500 to-blue-600 text-white font-bold text-lg">
                                        ...
                                        <div className="flex-1 min-w-0">
                                            <div className="flex items-center justify-between">
                                                <p className="font-semibold text-gray-900 truncate">{conv.client_nom}</p>
                                        ...
                                        {conv.non_lus > 0 && (
                                            <span className="flex h-5 w-5 items-center justify-center rounded-full bg-indigo-600 text-xs font-bold text-white shrink-0">
                                                {conv.non_lus}
                                            </span>
                                        )}
                                    </Link>
                                        ...
                                        {messagesState.length === 0 ? (
                                            ...
                                            messagesState.map((message) => (
                                                <div key={message.id} className={`flex items-end gap-3 ${message.isMine ? 'justify-end' : 'justify-start'}`}>
                                                    ...
                                                    <div className={`relative max-w-[70%] space-y-1 px-5 py-3 shadow-sm ${message.isMine ? 'bg-indigo-600 text-white rounded-tl-3xl rounded-tr-3xl rounded-bl-3xl' : 'bg-gray-100 text-gray-900 rounded-tl-3xl rounded-tr-3xl rounded-br-3xl'}`}>
                                                        {message.isMine ? (
                                                            <span className="absolute right-[-8px] top-4 h-0 w-0 border-y-8 border-y-transparent border-l-8 border-l-indigo-600" />
                                                        ) : (
                                                        ...
                                                        <div className={`mt-2 text-[11px] ${message.isMine ? 'text-indigo-100/80' : 'text-gray-500'}`}>
                                                            {message.date}
                                                        </div>
                                                        {message.call_session_id && (message.type === 'call_audio' || message.type === 'call_video') && (
                                                            <Link
                                                                ...
                                                                className="inline-flex items-center rounded-full bg-white px-3 py-1 text-[11px] font-semibold text-indigo-700 ring-1 ring-indigo-200 hover:bg-indigo-50"
                                                            >
                                                                Rejoindre l'appel
                                                            </Link>
                                                        )}
                                                    ...
                                                    {message.isMine && (
                                                        <div className="flex h-8 w-8 items-center justify-center rounded-full bg-indigo-500 text-xs font-semibold uppercase text-white">
                                                            V
                                                        </div>
                                                    )}
                                                </div>
                                    ...
                                    <form onSubmit={handleSend} className="border-t border-gray-100 px-6 py-4">
                                        ...
                                        <div className="space-y-2">
                                            <textarea
                                                ...
                                                className="w-full min-h-[110px] rounded-2xl border border-gray-200 bg-white p-4 text-gray-900 shadow-sm focus:border-indigo-500 focus:outline-none focus:ring-2 focus:ring-indigo-100"
                                            />
                                            {data.attachment && (
                                        ...
                                        <div className="mt-4 flex justify-end">
                                            <Button type="submit" disabled={sending || (data.contenu.trim() === '' && !data.attachment)} className="bg-gradient-to-r from-indigo-600 to-blue-600 hover:from-indigo-700 hover:to-blue-700">
